...
import com.sva.model.MediaModel;
...
public class ApiController
{
    ...
    public Map<String, Object> getMessages(@RequestBody Map<String,List<String>> map)
    {
        ...
        Collection<Map<String, Object>> mediaGroupList = new ArrayList<Map<String, Object>>();
        Map<String, Object> modelMap = new HashMap<String, Object>(2);
        ...
        try
        {
            ...
            if (!areaList.isEmpty())
            {
                for (int i = 0; i < areaList.size(); i++)
                {
                    ...
                    List<MediaGroupModel> mediaListTemp = daoMsg.getMediaByAreaId(areaId);
                    List<Map<String, Object>> result = formatMediaData(mediaListTemp);
                    mediaGroupList.addAll(result);
        ...
        modelMap.put("medias", mediaGroupList);
        return modelMap;
    }
    
    /** 
     * @Title: formatMediaData 
     * @Description: 对从数据库获取的多媒体文件列表进行格式化
     * @param input
     * @return 
     */
    private List<Map<String, Object>> formatMediaData(List<MediaGroupModel> input){
        List<Map<String, Object>> results = new ArrayList<Map<String, Object>>();
        for(MediaGroupModel item : input){
            Map<String, Object> temp = new HashMap<String, Object>();
            temp.put("id", item.getId());
            temp.put("name", item.getName());
            temp.put("plays", item.getPlays());
            temp.put("playType", item.getPlayType());
            List<MediaModel> mediaList = item.getMedias();
            Map<Integer, Object> mapper = new HashMap<Integer, Object>();
            for(MediaModel m:mediaList){
                switch(m.getPurpose()){
                    ...
                    // 轮播
                    case 3:
                        if(mapper.containsKey(3)){
                            @SuppressWarnings("unchecked")
                            List<Object> t = (List<Object>) mapper.get(3);
                            t.add(m);
                        }else{
            ...
            temp.put("preview", mapper.get(0));
            temp.put("introduce", mapper.get(4));
            // 1:音频，2：视频
            if(item.getPlayType().equals(1)){
                temp.put("loop", mapper.get(3));
                temp.put("sound", mapper.get(1));
            }else{
                temp.put("movie", mapper.get(2));
            }
            results.add(temp);
        }
        return results;
