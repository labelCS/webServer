...
function paramValue(){
	$.get("/sva/content/api/getData3?t="+Math.random(),function(data){
		if(!data.error){
			var a = data.data;
			$("#placeSel").val(Math.floor(a[0].floorNo1/10000));
    		var placeId = $("#placeSel").val();
			$.post("/sva/heatmap/api/getFloorsByMarket",{placeId:placeId}, function(data){
				if(!data.error){
					contentshow.updateFloorList1("zSel",data.data);
					$("#zSel").val(a[0].floorNo1);	
					}
			});
			...
			$("#radiusSel").val(a[0].radiusSel1);
			$("#radiusSel1").val(a[0].radiusSel2);
			$("#radiusSel2").val(a[0].radiusSel3);
			$("#radiusSel4").val(a[0].radiusSel4);

			$("#densitySel").val(a[0].densitySel1);
			$("#densitySel1").val(a[0].densitySel2);
			$("#densitySel2").val(a[0].densitySel3);
